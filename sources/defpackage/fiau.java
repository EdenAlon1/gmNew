...
public final class fiau extends fibt {
    ...
    public final PrivateKey a(PrivateKeyInfo privateKeyInfo) {
        return new fias(privateKeyInfo);
    }

    @Override // defpackage.fhsh
    public final PublicKey b(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        return new fiat(subjectPublicKeyInfo);
    }

    @Override // java.security.KeyFactorySpi
    public final KeySpec engineGetKeySpec(Key key, Class cls) {
        if (key instanceof fias) {
            if (PKCS8EncodedKeySpec.class.isAssignableFrom(cls)) {
                return new PKCS8EncodedKeySpec(key.getEncoded());
            }
        } else {
            if (!(key instanceof fiat)) {
    ...
    public final Key engineTranslateKey(Key key) {
        if ((key instanceof fias) || (key instanceof fiat)) {
            return key;
        }
