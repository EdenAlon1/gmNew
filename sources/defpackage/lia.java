...
public class lia extends InputStream implements DataInput, InputStreamRetargetInterface {
    ...
    public lia(InputStream inputStream) {
    ...
    public final long readLong() {
        ...
        if (this.c == ByteOrder.LITTLE_ENDIAN) {
            long j = read2;
            return (read8 << 56) + (read7 << 48) + (read6 << 40) + (read5 << 32) + (read4 << 24) + (read3 << 16) + (j << 8) + read;
        }
        if (this.c != ByteOrder.BIG_ENDIAN) {
            ByteOrder byteOrder = this.c;
            Objects.toString(byteOrder);
        ...
        long j2 = read3 << 40;
        return (read << 56) + (read2 << 48) + j2 + (read4 << 32) + (read5 << 24) + (read6 << 16) + (read7 << 8) + read8;
    }
    ...
    public lia(InputStream inputStream, ByteOrder byteOrder) {
        DataInputStream dataInputStream = new DataInputStream(inputStream);
        this.a = dataInputStream;
        dataInputStream.mark(0);
        this.b = 0;
        this.c = byteOrder;
        this.d = inputStream instanceof lia ? ((lia) inputStream).d : -1;
    }
