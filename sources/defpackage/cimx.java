package defpackage;

/* compiled from: PG */
/* loaded from: classes7.dex */
public final class cimx extends ciox {
    public final String a;
    public final String b;

    public cimx(String str, String str2) {
        if (str == null) {
            throw new NullPointerException("Null contentTitle");
        }
        this.a = str;
        if (str2 == null) {
            throw new NullPointerException("Null contentText");
        }
        this.b = str2;
    }

    @Override // defpackage.ciox
    public final String a() {
        return this.b;
    }

    @Override // defpackage.ciox
    public final String b() {
        return this.a;
    }

    public final boolean equals(Object obj) {
        if (obj == this) {
            return true;
        }
        if (obj instanceof ciox) {
            ciox cioxVar = (ciox) obj;
            if (this.a.equals(cioxVar.b()) && this.b.equals(cioxVar.a())) {
                return true;
            }
        }
        return false;
    }

    public final int hashCode() {
        return ((this.a.hashCode() ^ 1000003) * 1000003) ^ this.b.hashCode();
    }

    public final String toString() {
        return "MessageFailureNotificationContent{contentTitle=" + this.a + ", contentText=" + this.b + "}";
    }
}
