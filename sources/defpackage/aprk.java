package defpackage;

import com.google.android.apps.messaging.shared.api.messaging.MessageId;
import com.google.android.apps.messaging.shared.api.messaging.conversation.ConversationId;

/* compiled from: PG */
/* loaded from: classes6.dex */
public final class aprk implements aprm, alye {
    public final apip a;
    public final alye b;
    public final apld c;
    public final aqge d;
    private final ffbz e = ffca.a(new ffix() { // from class: aprj
        @Override // defpackage.ffix
        public final Object invoke() {
            aprk aprkVar = aprk.this;
            alxs a = aprkVar.b.a();
            a.getClass();
            apld apldVar = aprkVar.c;
            return aprkVar.a.a((apfl) a, aprkVar, apldVar, aprkVar.d);
        }
    });

    public aprk(apip apipVar, alye alyeVar, apld apldVar, aqge aqgeVar) {
        this.a = apipVar;
        this.b = alyeVar;
        this.c = apldVar;
        this.d = aqgeVar;
    }

    @Override // defpackage.alye
    public final elfl A(MessageId messageId) {
        return this.b.A(messageId);
    }

    @Override // defpackage.alye
    public final elfl B(MessageId messageId, String str, fjay fjayVar) {
        return this.b.B(messageId, str, fjayVar);
    }

    @Override // defpackage.alye
    public final elfl C(MessageId messageId) {
        return this.b.C(messageId);
    }

    @Override // defpackage.alye
    public final elfl D(MessageId messageId) {
        return this.b.D(messageId);
    }

    @Override // defpackage.alye
    public final elfl E(MessageId messageId) {
        return this.b.E(messageId);
    }

    @Override // defpackage.alye
    public final elfl F(alyv alyvVar) {
        return this.b.F(alyvVar);
    }

    @Override // defpackage.alye
    public final elfl G(MessageId messageId) {
        return this.b.G(messageId);
    }

    @Override // defpackage.alye
    public final elfl H(MessageId messageId, int i) {
        return this.b.H(messageId, i);
    }

    @Override // defpackage.alye
    public final void I(MessageId messageId, int i) {
        this.b.I(messageId, i);
    }

    @Override // defpackage.alye
    public final void J(alxr alxrVar, int i) {
        this.b.J(alxrVar, i);
    }

    @Override // defpackage.alye
    public final elfl K(MessageId messageId, apyv apyvVar, cmrx cmrxVar, fjay fjayVar, apxz apxzVar) {
        return this.b.K(messageId, apyvVar, cmrxVar, fjayVar, apxzVar);
    }

    @Override // defpackage.alye
    public final elfl L(int i) {
        return this.b.L(5);
    }

    @Override // defpackage.alye
    public final elfl M(int i) {
        return this.b.M(i);
    }

    @Override // defpackage.alye
    public final elfl N() {
        return this.b.N();
    }

    @Override // defpackage.alye
    public final void O(MessageId messageId, int i) {
        this.b.O(messageId, i);
    }

    @Override // defpackage.alye
    public final void P(MessageId messageId, int i) {
        this.b.P(messageId, i);
    }

    @Override // defpackage.alye
    public final void Q(int i) {
        this.b.Q(i);
    }

    @Override // defpackage.alye
    public final elfl R(MessageId messageId) {
        return this.b.R(messageId);
    }

    @Override // defpackage.alye
    public final alxs a() {
        return (apio) this.e.a();
    }

    @Override // java.lang.AutoCloseable
    public final void close() {
        this.b.close();
    }

    @Override // defpackage.alye
    public final alxu d() {
        throw null;
    }

    @Override // defpackage.alye
    public final alxy e() {
        return this.b.e();
    }

    @Override // defpackage.alye
    public final alym f() {
        throw null;
    }

    @Override // defpackage.alye
    public final ConversationId g() {
        return this.b.g();
    }

    @Override // defpackage.alye
    public final aqge h(MessageId messageId) {
        return this.b.h(messageId);
    }

    @Override // defpackage.alye
    public final aqge i() {
        return this.b.i();
    }

    @Override // defpackage.alye
    public final aqge j() {
        return this.b.j();
    }

    @Override // defpackage.alye
    public final ctbx k(alyd alydVar) {
        throw null;
    }

    @Override // defpackage.alye
    public final /* synthetic */ elfl l(MessageId messageId, engw engwVar) {
        return alyb.a();
    }

    @Override // defpackage.alye
    public final elfl m(MessageId messageId, apyv apyvVar, cmrx cmrxVar, fjay fjayVar) {
        return this.b.m(messageId, apyvVar, cmrxVar, fjayVar);
    }

    @Override // defpackage.alye
    public final elfl n(MessageId messageId) {
        return this.b.n(messageId);
    }

    @Override // defpackage.alye
    public final elfl o(engw engwVar) {
        return this.b.o(engwVar);
    }

    @Override // defpackage.alye
    public final elfl p() {
        return this.b.p();
    }

    @Override // defpackage.alye
    public final elfl q(engw engwVar) {
        return this.b.q(engwVar);
    }

    @Override // defpackage.alye
    public final elfl r(engw engwVar, cgrr cgrrVar) {
        return this.b.r(engwVar, cgrrVar);
    }

    @Override // defpackage.alye
    public final elfl s(engw engwVar) {
        return this.b.s(engwVar);
    }

    @Override // defpackage.alye
    public final elfl t() {
        return this.b.t();
    }

    @Override // defpackage.alye
    public final elfl u(MessageId messageId) {
        return this.b.u(messageId);
    }

    @Override // defpackage.alye
    public final elfl v() {
        return this.b.v();
    }

    @Override // defpackage.alye
    public final elfl w() {
        return this.b.w();
    }

    @Override // defpackage.alye
    public final elfl x() {
        throw null;
    }

    @Override // defpackage.alye
    public final elfl y() {
        return this.b.y();
    }

    @Override // defpackage.alye
    public final elfl z(MessageId messageId) {
        return this.b.z(messageId);
    }
}
