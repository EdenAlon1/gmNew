...
final class e extends FileChannel implements SeekableByteChannel {
    ...
    public static FileChannel i(FileChannel fileChannel, Set set, Path path) {
        if (fileChannel instanceof e) {
            fileChannel = ((e) fileChannel).a;
        }
        return new e(fileChannel, set.contains(C.DELETE_ON_CLOSE), set.contains(C.APPEND), path);
    }

    public static FileChannel p(FileChannel fileChannel) {
    ...
    public final FileChannel truncate(long j) {
        return p(this.a.truncate(j));
    }

    @Override // java.nio.channels.FileChannel
    public final FileChannel position(long j) {
        return p(this.a.position(j));
    }
